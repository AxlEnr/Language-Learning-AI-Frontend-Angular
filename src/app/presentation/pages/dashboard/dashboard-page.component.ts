...
@Component({
  ...
  template: `
    <div class="page container" *ngIf="dashboard">
      <div class="welcome-section"><h2>Welcome back, {{ dashboard.user.name }}!</h2><p class="text-secondary">Keep up the great work on your learning journey.</p></div>
      ...
      <div class="mt-4"><h3>Your Skills</h3>
        <div class="skills-grid mt-2">
          <div class="skill-card" *ngFor="let skill of dashboard.skills">
            <div class="flex justify-between"><span class="skill-name">{{ SKILL_LABELS[skill.skill] || skill.skill }}</span><span class="skill-level text-primary">Level {{ skill.level }}</span></div>
            <div class="progress-bar mt-1"><div class="progress-fill" [style.width.%]="skill.level"></div></div>
          </div>
        </div>
      </div>
      <div class="mt-4" *ngIf="recommendedLesson">
        <h3 class="mb-2">Recommended for You</h3>
        <div class="card card-hover">
          <div class="flex justify-between items-center">
            <div><h4>{{ recommendedLesson.title }}</h4><span class="badge mt-1">{{ LESSON_TYPE_LABELS[recommendedLesson.type] || recommendedLesson.type }}</span></div>
            <a [routerLink]="['/lessons', recommendedLesson.id]" class="btn btn-primary btn-sm">Start Lesson</a>
          </div>
        </div>
      </div>
      <div class="mt-4 text-center"><a routerLink="/modules" class="btn btn-outline">Browse All Modules</a></div>
    </div>
    <div class="page container text-center mt-4" *ngIf="!dashboard"><div class="spinner" style="width:32px;height:32px;margin:0 auto;"></div></div>
  `,
  styles: [`
...
export class DashboardPageComponent implements OnInit {
  dashboard: Dashboard | null = null; recommendedLesson: Lesson | null = null;
  SKILL_LABELS = SKILL_LABELS; LESSON_TYPE_LABELS = LESSON_TYPE_LABELS;
  ...
  ngOnInit(): void {
    this.dashboardUseCase.getDashboard().subscribe({ next: (data) => this.dashboard = data, error: () => {} });
    this.aiUseCase.recommendLesson().subscribe({ next: (lesson) => this.recommendedLesson = lesson, error: () => {} });
  }
}
